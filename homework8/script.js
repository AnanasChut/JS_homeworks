...
Это происходит потому, что в 5 задание событие click работает только для элементов ul и у нас есть внутри if,который проверяет, является ли свойство .target.tagName, которое возвращает строку, содержащую имя тега элемента,на который был сделан клик -> "BUTTON". В связи с этим, 3-e задание, в котором есть document.addEventListener,удовлетворяет всем click.
